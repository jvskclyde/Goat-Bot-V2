...
module.exports = {
  ...
  async makeApiRequest(encodedPrompt, uid, a) {
    try {
      const response = await axios.get(`https://sandipbaruwal.onrender.com/gpt2?prompt=${encodedPrompt}&uid=${uid}`);
  ...
  async handleCommand({ message, event, args, api }) {
    try {
      ...
      if (args[0] === 'draw') {
        ...
        const puti = model || "2";
        const baseURL = `https://sandipbaruwal.onrender.com/sdxl?prompt=${promptText}&model=${puti}`;
